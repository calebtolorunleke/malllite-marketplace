  ...
  return (
    <footer className="bg-[#0b1220] text-gray-300 mt-24 border-t border-white/10">
      <div className="max-w-7xl mx-auto px-6 py-14 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-10">
        ...
        {/* BRAND */}
        <div>
          <h2 className="text-2xl font-black text-yellow-400 tracking-wide">
            MallLite
          </h2>
        ...
        <div>
          ...
          <ul className="space-y-3 text-sm">
            {["Home", "Shop", "Categories", "Cart"].map((item) => (
              <li
                key={item}
                className="cursor-pointer hover:text-yellow-400 transition w-fit"
              >
                {item}
              </li>
            ))}
